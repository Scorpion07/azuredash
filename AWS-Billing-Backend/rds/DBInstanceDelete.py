...
def DBInstanceDelete(event):

    instanceidlist = list(event['instance_id'])
    regionlist = list(event['region'])
    i=0
        ...
        for count in range(0,len(instanceidlist)):
            ...
            if instance_id["SkipSnapshot"] == False:
                try:
                    ...
                    if e.response['Error']['Code'] == 'DBClusterNotFoundFault' or e.response['Error']['Code'] == 'InvalidDBClusterStateFault' or e.response['Error']['Code'] == 'InvalidParameterCombination':
